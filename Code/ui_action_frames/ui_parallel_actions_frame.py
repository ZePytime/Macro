...
class ParallelActionsFrame:
    ...
    def __init__(self, parent_frame: tk.Frame, container_manager: ContainerManager) -> None:
        ...
        :param container_manager: Classe permettant de gérer les conteneurs 
            comme loop et parallel_actions.
        ---------------------------------------------------------------------
        ...
        # et parallel_actions, elle nous sera utile pour vérifier les noms 
        # de parallel_actions déjà utilisés et lui donner les nouveaux noms.
        # -------------------------------------------------------------------
    ...
    def check(self) -> Union[Tuple[str, list[SpecialKeys], str], bool]:
        """
        On supprime aussi les doublons dans les touches normales.
        On vérifie qu'il y a au moins une touche sélectionnée.
        On vérifie que le nom est valide (non vide, pas trop long, 
        pas déjà utilisé).

        :return: Un tuple contenant les touches normales, 
            les touches spéciales et le nom si tout est valide, sinon False.
        -----------------------------------------------------------------------
        Removes duplicates in normal keys as well.
        Checks that at least one key is selected.
        Checks that the name is valid (not empty, not too long, 
        not already used).

        :return: A tuple containing normal keys, 
            special keys and the name if everything is valid, otherwise False.
        """

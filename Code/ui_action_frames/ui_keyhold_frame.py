...
from enums import SPECIAL_KEYS_LAYOUT, SpecialKeys
...
class KeyHoldFrame:
    ...
    def __init__(self, parent_frame: tk.Frame) -> None:
        ...
        info_label_keys = tk.Label(
            parent_frame, 
            text='Enter all the keys you want to use:', 
        ...
        frm_special_keys = tk.Frame(parent_frame, bg=W_VAR.NEUTRAL_800)
        ...
        for row, list_btn in enumerate(SPECIAL_KEYS_LAYOUT):
            ...
            frame_btn = tk.Frame(frm_special_keys, bg=W_VAR.NEUTRAL_800)
    ...
    def toggle_special_key(self, name: str) -> None:
        ...
        du bouton afin que l'utilisateur sache quelles touches sont 
        sélectionnées et ajoute la touche à la liste des touches spéciales 
        sélectionnées.
        ...
        if name in self._selected_special_keys:
            ...
            self._special_key_buttons[name].config(bg=W_VAR.NEUTRAL_700)
        else:
            self._selected_special_keys.add(name)
            self._special_key_buttons[name].config(bg=W_VAR.BUTTON_1)


    def check(self) -> Union[Tuple[str, list[SpecialKeys], float], bool]:
        """
        Vérifie que le temps saisi est un nombre, n'est pas trop élevé, 
        moins de 3 décimales, et qu'il y a au moins une touche sélectionnée. 
        On supprime aussi les doublons dans les touches normales.

        :return: Un tuple (str de touches normales, liste de touches spéciales, 
            float temps) si ok, sinon False et affiche un message d'erreur.
        ------------------------------------------------------------------------
        Checks that the entered time is a number, not too high, 
        less than 3 decimals, and that there is at least one key selected. 
        Also removes duplicates in normal keys.

        :return: A tuple (str of normal keys, list of special keys, float time) 
            if ok, otherwise False and displays an error message.
        """
        ...
        # conserve le premier exemplaire de chaque caractère en respectant 
        # l'ordre.
        # ---------------------------------------------------------------------
        # Removes duplicate characters in the normal keys Entry
        # keeps the first occurrence of each character while preserving order.
        seen = set()
        deduped_chars = []
        ...
        # updates the Entry to reflect the cleaned value.

...
# On donne les dimensions de l'écran principal de l'utilisateur à W_VAR.
# -----------------------------------------------------------------------
# Set the user's main screen dimensions in W_VAR.
W_VAR.screen_width = window.winfo_screenwidth()
W_VAR.screen_height = window.winfo_screenheight()

# On définit les propriétés de la fenêtre.
# -----------------------------------------
# Set window properties.
window.title("Zecron")
icon = tk.PhotoImage(file="Snake_Logo.png")
window.iconphoto(True, icon)
window.geometry("370x550")
window.minsize(W_VAR.window_width, W_VAR.window_height)
window.config(bg=W_VAR.NEUTRAL_900)




# On crée un cadre principal pour contenir tout le contenu de l'application.
# ---------------------------------------------------------------------------
# We create a main frame to contain all the content of the application.
main_frm = tk.Frame(window, bg=W_VAR.NEUTRAL_900)
...
menu_bt_frm = tk.Frame(main_frm, bg=W_VAR.NEUTRAL_900)
menu_bt_frm.grid(row=0, column=0, padx=5, pady=5)

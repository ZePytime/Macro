...
SEQUENCES_FILE_PATH = Path("Data/save_act_sequence.pkl")
...
class SavedActionSequence:
    ...
    def recreate_action_sequence(self, action_dict_manager: ActionManager, leave_current_container: callable, container_manager: ContainerManager) -> None:
        """
        Cette fonction prend les paramètres suivants :

        :param action_dict_manager: l'instance de ActionManager pour ajouter les actions à action_dict.
        ...
        Si l'action est un conteneur (loop, parallel actions), on effectue le même processus, mais après, on 
        ...
        :param action_dict_manager: the instance of ActionManager used to add actions to action_dict.
        ...
        If the action is a container (loop, parallel actions), the same process is applied, but afterward,
        ...
        for action_entry in self.sequence:
            match action_entry[0]:
                ...
                case ActionType.CLICK_RIGHT.value:
                    action_dict_manager.add_right_click(action_entry[1], action_entry[2], container_manager.is_in_container)

                case ActionType.CLICK_LEFT.value:
                    action_dict_manager.add_left_click(action_entry[1], action_entry[2], container_manager.is_in_container)
                ...
                case ActionType.PARALLEL_ACTIONS.value:
                    action_dict_manager.add_parallel_actions(action_entry[1], action_entry[2], action_entry[3], container_manager.is_in_container)
                ...
                case ActionType.LEFT_CONTAINER.value:
...
def save_sequence(name: str, action_list: list[list[str]]) -> Union[bool, Tuple[str, int]]:
    ...
    if len(name) > 25:
        return "Name cannot exceed 25 characters", 1

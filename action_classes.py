...
# donner la clé correspondante et le dictionnaire renvoie l'élément adapté pour simuler l'appui.
# -----------------------------------------------------------------------------------------------------
# We create a dictionary that, for each special key as well as right-click and left-click,
# contains the corresponding pynput element. This allows us, when simulating a key press,
# to simply provide the corresponding key and the dictionary returns the appropriate element.
SP_KEY_DICT = {
...
def position_mouse() -> Tuple[int, int]:
    ...
    :return: un tuple contenant les positions x et y de la souris de l'utilisateur.
    --------------------------------------------------------------------------------
...
class KeyPosition:
    """
    Cette classe permet de capturer la position de la souris de l'utilisateur 
    et de l'inscrire dans les champs de texte des interfaces click_ui et move_ui.
    La position de la souris est capturée lorsque l'utilisateur appuie simultanément 
    sur les touches "X" et "V" (ces touches peuvent être modifiées).
    ---------------------------------------------------------------------------------
    This class allows capturing the user's mouse position 
    and inserting it into the text fields of the click_ui and move_ui interfaces.
    ...
    # pressions de touches ou non.
    # ---------------------------------------------------------------------------
    # This variable indicates whether we are currently listening to key presses.
    is_listening = False
    ...
    def __init__(self, set_coordinate_click: Callable[[Tuple[int, int], str], None], set_coordinate_move: Callable[[Tuple[int, int], str], None]) -> None:
        """
        On démarre l'observation des touches pressées par l'utilisateur pour capturer la position de la souris.
        ...
        # ---------------------------------------------------------------------------
        # Set is_listening to True to indicate that we are listening to key presses.
        ...
        # On crée un set qui contiendra les touches de captures lorsqu'elles sont pressées.
        # ----------------------------------------------------------------------------------
        # Create a set that will store pressed capture keys.
        self.pressed_position_keys = set()

        # On choisit d'observer tous les appuis et les relâchements de touches.
        # ----------------------------------------------------------------------
    ...
    def on_press(self, key) -> None:
        ...
        # On vérifie si les deux touches pour capturer la position de la souris sont pressées simultanément.
        # ---------------------------------------------------------------------------------------------------
        ...
        if len(self.pressed_position_keys) == len(KeyPosition.position_capture_keys):
            # Si c'est le cas, on vide le set et on appelle set_coordinate 
    ...
    def on_release(self, key) -> None:
        ...
        # On vérifie si c'est une des touches permettant de capturer la position.
        # ------------------------------------------------------------------------
        # Check if it is one of the keys used to capture the position.
...
class KeyLoggerApp:
    ...
    utilisée que par ParallelActions, Loop et ActionManager.
    -------------------------------------------------------------------------------------
    This class allows stopping the execution of the action sequence when 
    the "y" and "j" keys are pressed simultaneously (these keys can be modified). 
    It is only used by ParallelActions, Loop, and ActionManager.
    """

    # Variable permettant de vérifier si les touches pour arrêter l'exécution 
    # de la suite d'actions ont été pressées. Elle permet au conteneur (ParallelActions, Loop) 
    # ou à start_execution dans ActionManager de ne pas exécuter de nouvelles actions 
    # lorsque les touches d'arrêt ont été pressées.
    # -----------------------------------------------------------------------------------------
    ...
    def __init__(self, cancel_task: Callable[[], None]) -> None:
        """
        Cette fonction permet d'initialiser les variables pour le fonctionnement de la classe KeyLoggerApp,
        et démarre l'observation des touches pressées par l'utilisateur pour stopper l'exécution de la suite d'actions.

        :param cancel_task: fonction permettant de stopper les actions en cours.
        ----------------------------------------------------------------------------------------------------------------
        This function initializes the variables required for the KeyLoggerApp class,
        and starts listening to user key presses to stop the execution of the action sequence.

        :param cancel_task: function used to stop the current actions.
        """

        # On récupère cancel_task pour l'appeler lorsque les touches d'arrêt sont pressées simultanément.
        # ------------------------------------------------------------------------------------------------
        # Store cancel_task to call it when the stop keys are pressed simultaneously.
        self.cancel_task = cancel_task

        # On crée un set qui contiendra les touches d'arrêt lorsqu'elles sont pressées.
        # ------------------------------------------------------------------------------
        # Create a set that will contain the stop keys when they are pressed.
    ...
    def on_press(self, key) -> None:
        ...
        if len(self.pressed_stop_keys) == len(KeyLoggerApp.stop_keys):
            # Si c'est le cas, on met stop_run à True pour indiquer que les touches d'arrêt ont été pressées.
            # ------------------------------------------------------------------------------------------------
            # If so, set stop_run to True to indicate that stop keys were pressed.
    ...
    def on_release(self, key) -> None:
        ...
        ---------------------------------------------------------------
        This function is called when a key is released. 
        It removes the stop keys from pressed_stop_keys when they are released.
        ...
        # On vérifie si c'est une des touches permettant l'arrêt de l'exécution de la suite d'actions.
        # ---------------------------------------------------------------------------------------------
        # Check if it is one of the stop keys.
...
def press_keys(special_keys: list, keys: str) -> None:
    ...
    :param keys: une chaîne de caractères contenant les touches normales à actionner.
    ----------------------------------------------------------------------------------
...
def release_keys(special_keys: list, keys: str) -> None:
    ...
    :param keys: une chaîne de caractères contenant les touches normales à relâcher.
    ---------------------------------------------------------------------------------
...
class KeyPress(Action):
    ...
    def __init__(self, keys:str, special_keys:list, time_wait:float) -> None:
        """
        Initialisation des paramètres nécessaires à la pression de touches pendant un certain temps.
        
        :param keys: une chaîne de caractères contenant les touches normales à presser.
        :param special_keys: une liste de touches spéciales à presser.
        :param time_wait: la durée pendant laquelle les touches doivent être pressées.
        ---------------------------------------------------------------------------------------------
        Initializes the parameters required to press keys for a given duration.
        ...
        # On stocke le temps d'attente en secondes pour pouvoir l'afficher dans text.
        # ----------------------------------------------------------------------------
        # Store wait time in seconds to display it in text.
        self.time_wait_s = time_wait
        # On multiplie le temps d'attente par 1000 car il est de base compté en millisecondes.
        # -------------------------------------------------------------------------------------
        # Convert wait time to milliseconds.
        self.time_wait_ms = int(time_wait*1000)

        # On supprime les caractères en double de la chaîne de caractères qui contient toutes les touches normales.
        # ----------------------------------------------------------------------------------------------------------
        # Remove duplicate characters from the string containing all regular keys.
        self.keys = "".join(list(set(keys)))
        
        self.special_keys = special_keys

        # On crée une liste de chaînes de caractères contenant les noms des touches spéciales 
        # à partir de la liste de touches spéciales donnée en paramètre, pour pouvoir les afficher dans text.
        # ----------------------------------------------------------------------------------------------------
        ...
        # On trie les noms des touches spéciales par ordre de taille pour la fonction text.
        # ----------------------------------------------------------------------------------
    ...
    def run(self) -> int:
        """
        Cette fonction appelle press_keys pour actionner les touches que l'utilisateur a entrées au préalable, 
        puis retourne le temps d'attente pendant lequel ces touches doivent être pressées. 
        (Ensuite, la fonction qui a appelé cette fonction se chargera, après ce temps, d'appeler release_keys.)
        --------------------------------------------------------------------------------------------------------
    ...
    def release_keys(self) -> None:
        """
        Cette fonction appelle release_keys pour relâcher les touches qui ont été précédemment actionnées.
        ----------------------------------------------------------------------------------------------------
        This function calls release_keys to release the keys that were previously pressed.
        """
    ...
    def text(self) -> str:
        """
        Cette fonction retourne une chaîne de caractères pour représenter l'action dans l'arbre des actions. 
...
class Move(Action):
    """
    Cette classe permet de déplacer la souris à l'endroit choisi par l'utilisateur ou 
    de la déplacer d'une certaine distance dans une direction choisie par l'utilisateur.
    -------------------------------------------------------------------------------------
    ...
    def __init__(self, pos_x:int, pos_y:int, move_type:str):
        """
        On prend la position à laquelle doit se déplacer la souris et le type de déplacement.

        :param pos_x: la position x à laquelle doit se déplacer la souris ou la distance à 
            laquelle doit se déplacer la souris en x.
        :param pos_y: la position y à laquelle doit se déplacer la souris ou la distance à 
            laquelle doit se déplacer la souris en y.
        :param move_type: le type de déplacement, soit "absolute" pour déplacer la 
            souris à une position précise, soit "relative" pour déplacer la souris d'une 
            certaine distance de sa position initiale en x y.
        --------------------------------------------------------------------------------------
...
class Write(Action):
    ...
    def __init__(self, payload_text:str) -> None:
        ...
        :param payload_text: le texte que l'utilisateur souhaite faire écrire.
        ------------------------------------------------------------------------
...
class Scroll(Action):
    ...
    def __init__(self, step:int, direction:str) -> None:
        ...
        :param direction: la direction dans laquelle l'utilisateur souhaite scroller.
        ------------------------------------------------------------------------------
...
class Wait(Action):
    ...
    def __init__(self, time_wait:float) -> None:
        """
        Initialisation des paramètres nécessaires à la création d'un temps d'attente.

        :param time_wait: le temps que l'utilisateur souhaite attendre entre deux actions, en secondes.
        ------------------------------------------------------------------------------------------------
        ...
        # On multiplie le temps d'attente par 1000 car il est de base compté en millisecondes.
        # -------------------------------------------------------------------------------------
        # Multiply the wait time by 1000 because it is internally handled in milliseconds.
        self.time_wait_ms = int(time_wait*1000)
        # On stocke aussi le temps d'attente en secondes pour pouvoir l'afficher dans text().
        # ------------------------------------------------------------------------------------
        # Also store the wait time in seconds so it can be displayed in text().
        self.time_wait_s = time_wait
...
class Loop(Action):
    ...
    d'affilée.
    ----------------------------------------------------------------------------------------
    This class allows executing a sequence of actions a specified number of times in a row.
    """

    def __init__(self, window: tk.Tk, nb_turns:int, name:str) -> None:
        """ 
        Initialisation des paramètres nécessaires au fonctionnement de la boucle.

        :param window: La fenêtre de l'interface, nécessaire pour pouvoir appeler des fonctions 
            après un certain temps sans bloquer l'exécution du programme.
        :param nb_turns: Le nombre d'exécutions que la boucle doit effectuer avec la suite d'actions.
        :param name: Le nom de la boucle.
        ----------------------------------------------------------------------------------------------
        Initializes the parameters required for the loop to function.

        :param window: The UI window, required to call functions after a delay 
            without blocking program execution.
        :param nb_turns: The number of times the loop should execute the action sequence.
        ...
        # On crée une variable turn_counter qui va s'incrémenter à chaque 
        # tour de boucle jusqu'à atteindre le nombre demandé.
        # --------------------------------------------------------------------------
        # Counter that increments on each loop iteration until reaching the target.
        self.turn_counter = 0
    ...
    def add_action(self, inst_act: Action) -> None:
        ...
        la boucle container_actions_dict avec pour clé next_container_action_index, puis on incrémente next_container_action_index.

        :param inst_act: l'instance de l'action à ajouter à la suite d'actions de la boucle.
        -----------------------------------------------------------------------------------------
        Takes an action instance as parameter and adds it to the loop action dictionary 
        container_actions_dict with key next_container_action_index, then increments next_container_action_index.

        :param inst_act: the action instance to add to the loop action sequence.
        """
    ...
    def add_action_container_tree(self, text: str, ui_action_tree: TreeUI, spacing: int) -> None:
        ...
        :param text: La chaîne de caractères représentant l'action qui a été ajoutée à la boucle.
        :param ui_action_tree: L'instance de la classe TreeUI qui nous permet d'avoir accès à la 
            frame (all_actions) dans laquelle nous devons ajouter le Checkbutton.
        :param spacing: L'incrémentation à laquelle doit être positionné le Checkbutton.
        ------------------------------------------------------------------------------------------
        ...
        # On crée la variable is_checked_act qui permettra de savoir si le Checkbutton 
        # est coché ou non.
        # -----------------------------------------------------------------------------
        # Variable used to track whether the Checkbutton is checked.
        is_checked_act = tk.IntVar(value=1)
        # On crée le Checkbutton et on l'affiche dans all_actions avec l'incrémentation.
        # -------------------------------------------------------------------------------
        # Create and display the Checkbutton in all_actions with indentation.
        ChkBt_act = tk.Checkbutton(ui_action_tree.all_actions.scrollable_frame, text=text, variable=is_checked_act, bg=ui_action_tree.w_var.color_1)
        ChkBt_act.grid(row=ui_action_tree.next_checkbutton_row+1, column=0, sticky="wn", padx=spacing)

        # On ajoute au dictionnaire qui va contenir tous les Checkbutton de la boucle (container_action_checkbuttons), 
        # une liste comportant en premier is_checked_act, puis le Checkbutton. Cette liste a pour clé tree_container_idx.
        # -------------------------------------------------------------------------------------------------------------
        # Add to the dictionary that will store all Checkbuttons of the loop (container_action_checkbuttons),
        # a list containing first is_checked_act, then the Checkbutton.
        ...
        # On incrémente la ligne où se trouvera la prochaine action (next_checkbutton_row).
        # ----------------------------------------------------------------------------------
        # Increment the row where the next action will be placed (next_checkbutton_row).
    ...
    def cancel_task(self) -> None:
        ...
        Elle est appelée soit par une instance de KeyLoggerApp, soit par le conteneur 
        dans lequel elle se trouve soit par ActionManager.
        ...
        its execution is stopped via cancel_task (it will apply the same logic).
    ...
    def run(self, advent_function: Callable[[None], None]=None, pause_between_actions: int=None) -> None:
        """
        Cette fonction, à chaque fois qu'elle est appelée, exécute un tour de boucle. 
        Si le nombre de tours requis a été atteint, elle appelle advent_function afin 
        de continuer l'exécution de la suite d'actions.
        ...
        :param advent_function: La fonction qui a appelé cette fonction, 
            ...
            continuer le reste de l'exécution de la suite d'actions après cette boucle.
        :param pause_between_actions: Le temps d'attente entre chaque action choisi par l'utilisateur.
        -------------------------------------------------------------------------------------------
        ...
        :param pause_between_actions: The delay between each action defined by the user.
        """
        
        # Si advent_function n'est pas None, cela signifie que c'est la première fois que run est appelée,
        # alors on stocke advent_function pour pouvoir l'utiliser à la fin et pause_between_actions pour 
        ...
        # Si c'est le cas, on appelle run_one_iteration pour exécuter un tour de boucle, et on incrémente turn_counter.
        # ---------------------------------------------------------------------------------------------------
        ...
        if self.turn_counter < self.nb_turns:
            ...
            # Sinon, on a atteint le nombre de tours requis, alors on réinitialise turn_counter 
            # pour pouvoir reexecuter la boucle si besoin, et on appelle advent_function pour continuer 
    ...
    def run_one_iteration(self):
        ...
        action de la boucle l'une après l'autre. Afin de savoir l'action en cours
        d'exécution.
        On utilise la variable current_container_action_index qui est incrémentée à chaque appel de cette 
        fonction.
        ...
        if self.current_container_action_index >= self.next_container_action_index or KeyLoggerApp.stop_run:
            # On a exécuté chaque action ou l'exécution a été arrêtée, alors on met current_container_action_index à 0 
            # pour refaire un tour si besoin. Puis, on appelle "run", qui va soit refaire un tour 
            # de boucle, soit appeler la fonction qui l'a initialement appelée.
            # ------------------------------------------------------------------------------------------
            ...
            # On incrémente current_container_action_index car on va exécuter l'action suivante.
            # --------------------------------------------------------------------
            # Move to next action.
            ...
            if list(self.container_action_checkbuttons[self.current_container_action_index-1])[0].get() == 0:
                ...
                # -----------------------------------------------------------------
                # Si l'action est un conteneur, on appelle sa méthode d'exécution.
                if isinstance(self.container_actions_dict[self.current_container_action_index-1], Loop) or isinstance(self.container_actions_dict[self.current_container_action_index-1], ParallelActions):
                    # On donne à run la fonction run_one_iteration pour qu'elle puisse 
                    # rappeler la fonction run_one_iteration une fois qu'elle a fini d'exécuter les actions 
                    # qu'elle contient et on donne aussi le temps de pause entre chaque action.
                    # ---------------------------------------------------------------------------
                    # We pass the run_one_iteration function to run so that it can
                    # call the run_one_iteration function again once it has finished executing the actions
                    # it contains, and we also pass the pause time between each action.
                    ...
                    # We retrieve the time to wait before calling the next action
                    # that it returns if it has one.
                    ...
                    # Empêche les bugs causés par la remise à zéro de current_container_action_index,
                    # qui sert de clé pour le dictionnaire.
                    # -----------------------------------------------------------------
                    # Guard to prevent invalid accesses after a stop.
                    # Prevents bugs caused by resetting current_container_action_index,
                    # which is used as a key for the dictionary.
                    if self.current_container_action_index <= 0: 
                        return
                    ...
                    # ------------------------------------------------------------------------------
                    # We check if the action that has just been executed was a KeyPress in order to
                    # call call_release_keys after the waiting time chosen by the user.
                    if isinstance(self.container_actions_dict[self.current_container_action_index-1], KeyPress):
                    ...
                    # rappeler run_one_iteration après le temps d'attente choisi par l'utilisateur.
                    # --------------------------------------------------------------------------
                    # We check if the action that has just been executed was a Wait in order to
                    # call run_one_iteration again after the waiting time chosen by the user.
                        ...
                        # nous appelons run_one_iteration pour exécuter l'action suivante.
                        # ------------------------------------------------------------------------------
                        # The action that we have just called does not require any waiting time, so
                        # we call run_one_iteration to execute the next action.
                        self.window.after(self.pause_between_actions, self.run_one_iteration)


    def call_release_keys(self) -> None:
        """
        Cette fonction appelle release_keys pour KeyPress afin de relâcher les touches précédemment 
        pressées, puis on appelle run_one_iteration pour exécuter l'action d'après dans la boucle.
        ----------------------------------------------------------------------------------------------
        This function calls release_keys for KeyPress in order to release the keys that were previously
        pressed, then calls run_one_iteration to execute the next action in the loop.
        """
        self.container_actions_dict[self.current_container_action_index-1].release_keys()
        self.window.after(self.pause_between_actions, self.run_one_iteration)
...
class ParallelActions(Action):
    """
    Cette classe permet d'exécuter une suite d'actions en même temps qu'une ou plusieurs touches 
    sont pressées.
    ---------------------------------------------------------------------------------------------
    This class allows executing a sequence of actions while one or more keys
    are pressed simultaneously.
    """

    def __init__(self, window: tk.Tk, keys:str, special_keys:list, name:str) -> None:
        """
        Initialisation des paramètres nécessaires au fonctionnement des actions simultanées.

        :param window: La fenêtre de l'interface, nécessaire pour pouvoir appeler des fonctions 
            après un certain temps sans bloquer l'exécution du programme.
        :param keys: une chaîne de caractères contenant les touches normales à 
            presser pendant l'exécution de la suite d'actions.
        :param special_keys: une liste de touches spéciales à presser pendant l'exécution de la suite d'actions.
        ...
        :param keys: A string containing the normal keys to press during the execution of the action sequence.
        :param special_keys: A list of special keys to press during the execution of the action sequence.
        ...
        # On supprime les caractères en double de la chaîne de caractères qui contient 
        # toutes les touches normales (keys).
        # -----------------------------------------------------------------------------
        ...
        # ----------------------------------------------------------------------------------------------------
        # Create a list of strings containing the names of the special keys
        # from the list of special keys passed as a parameter, so they can be displayed in text.
        special_key_labels = []
        for sp_key in self.special_keys:
            special_key_labels.append(sp_key.value)
        # On trie les noms des touches spéciales par ordre de taille pour la fonction text.
        ...
        # On stocke le nom de la suite d'actions simultanées pour pouvoir l'afficher dans text.
        # --------------------------------------------------------------------------------------
        # Store the name of the simultaneous action sequence so it can be displayed in text.
        self.name = name
        ...
        self.next_container_action_index = 0
        # Variable servant à savoir quelle action est en cours d'exécution
        # et si la suite d'actions des actions simultanées a fini son exécution.
        # -----------------------------------------------------------------------
        # Variable used to track which action is currently being executed
        # and whether the sequence of actions has finished executing.
        self.current_container_action_index = 0
        ...
        # On crée finished_running, qui va permettre à run de savoir si la liste d'actions a déjà 
        # été exécutée.
        # ------------------------------------------------------------------------------------------
        # Create finished_running, which allows run to know if the action list
    ...
    def add_action(self, instance: Action) -> None:
        """
        On prend l'instance de l'action en paramètre et on l'ajoute au dictionnaire 
        d'actions des actions paraleles container_actions_dict avec pour clé next_container_action_index, 
        puis on incrémente next_container_action_index.

        :param inst_act: l'instance de l'action à ajouter à la suite d'actions.
        ----------------------------------------------------------------------------
        Takes an action instance as a parameter and adds it to the dictionary
        of parallel actions (container_actions_dict) using next_container_action_index as the key,
    ...
    def add_action_container_tree(self, text: str, ui_action_tree: TreeUI, spacing: int) -> None:
        ...
        :param text: La chaîne de caractères représentant l'action qui a été ajoutée à ce conteneur.
        :param ui_action_tree: L'instance de la classe TreeUI qui nous permet d'avoir accès à la 
            frame (all_actions) dans laquelle nous devons ajouter le Checkbutton.
        :param spacing: L'incrémentation à laquelle doit être positionné le Checkbutton.
        ---------------------------------------------------------------------------------------------
        ...
        :param text: The string representing the action added to this container.
        :param ui_action_tree: The TreeUI instance that gives access to the
            frame (all_actions) where the Checkbutton must be added.
        :param spacing: The indentation level where the Checkbutton should be placed.
        """

        # On crée la variable is_checked_act qui permettra de savoir si le Checkbutton 
        # est coché ou non. 
        # -----------------------------------------------------------------------------
        # Create the is_checked_act variable to determine whether the Checkbutton
        # is checked or not.
        is_checked_act = tk.IntVar(value=1)
        # On crée le Checkbutton et on l'affiche dans all_actions avec l'incrémentation.
        # -------------------------------------------------------------------------------
        # Create the Checkbutton and display it in all_actions with indentation.
        ChkBt_act = tk.Checkbutton(ui_action_tree.all_actions.scrollable_frame, text=text, variable=is_checked_act, bg=ui_action_tree.w_var.color_1)
        ChkBt_act.grid(row=ui_action_tree.next_checkbutton_row+1, column=0, sticky="wn", padx=spacing)

        # On ajoute au dictionnaire qui va contenir tous les Checkbutton des actions paraleles (container_action_checkbuttons), 
        # une liste comportant en premier is_checked_act, et le Checkbutton cette liste a pour clé tree_container_idx.
        # ----------------------------------------------------------------------------------------------------------------------
        # Add to the dictionary containing all Checkbuttons of parallel actions (container_action_checkbuttons),
        # a list where the first element is is_checked_act and the second is the Checkbutton.
    ...
    def run(self, advent_function=None, pause_between_actions=None) -> None:
        """
        Cette fonction va lancer l'appui des touches choisies par l'utilisateur,
        puis exécuter les actions de la suite, puis à la fin de la suite d'actions, 
        elle va relâcher les touches et appeler la fonction qui l'a initialement appelée 
        pour continuer l'exécution de la suite d'actions après les actions simultanées.

        :param advent_function: la fonction qui a appelé cette fonction, pour pouvoir 
            l'appeler à la fin de l'exécution de la suite d'actions, et continuer 
            l'exécution de la suite d'actions après les actions simultanées.
        :param pause_between_actions: le temps d'attente entre chaque action choisi par l'utilisateur.
        -------------------------------------------------------------------------------------------
        ...
        :param advent_function: The function that called this one, so it can be
            called again after the sequence finishes.
        :param pause_between_actions: The delay between each action chosen by the user.
        """

        # Si advent_function n'est pas None, cela signifie que c'est la première fois que run est appelée.
        # -----------------------------------------------------------------------------------------------------
        # If advent_function is not None, it means this is the first call to run.
        if not advent_function is None:
            # On stocke advent_function pour pouvoir l'utiliser à la fin et pause_between_actions pour 
            # mettre une pause entre chaque action de la suite d'actions simultanées.
            # -------------------------------------------------------------------------------------
        ...
        if self.finished_running:
            ...
            release_keys(self.special_keys, self.keys)
            # On appelle advent_function pour continuer l'exécution de la suite d'actions après les actions simultanées.
            # -----------------------------------------------------------------------------------------------------------
            # Call advent_function to continue execution.
            self.window.after(self.pause_between_actions, self.advent_function)
            # On remet finished_running à False pour la prochaine fois que run sera appelée.
            # ---------------------------------------------------------------------------------
            # Reset finished_running for the next call.
            self.finished_running = False
        else:
            print("sfijgiuorfojgvojfnvoj")
            # Sinon, on appuie sur les touches choisies par l'utilisateur, et on appelle run_action pour exécuter la suite d'actions.
            # ------------------------------------------------------------------------------------------------------------------------
    ...
    def run_action(self):
        ...
        Cette fonction s'appelle elle-même de manière asynchrone pour exécuter chaque
        action de la suite l'une après l'autre. Afin de savoir l'action en cours
        d'exécution, on utilise la variable current_container_action_index qui est incrémentée à chaque appel de cette 
        fonction.
        Lorsqu'une action est un conteneur (Loop/ParallelActions), on appelle la méthode 
        run de ce conteneur qui s'occupe de l'exécution des actions qu'il contient, 
        puis quand il aura fini, il va rappeler la fonction start_execution, et l'exécution de 
        ...
        action in the sequence one after the other. To keep track of the action
        currently being executed, the variable current_container_action_index is used and is incremented
        at each call of this function.

        When an action is a container (Loop/ParallelActions), the method
        run of this container is called, which handles the execution of the actions it contains,
        then when it has finished, it will call back the start_execution function, and the execution of
        ParallelActions will resume where it had stopped.
        """
        ...
        if self.current_container_action_index >= self.next_container_action_index or KeyLoggerApp.stop_run:
            ...
            # so current_container_action_index is reset to 0 in order to execute them again if needed.
            self.current_container_action_index = 0
            # On met finished_running à True pour que run sache que la suite d'actions a déjà été exécutée.
            # -----------------------------------------------------------------------------------------------------
            # Set finished_running to True so that run knows that the action sequence has already been executed.
            self.finished_running = True
            # On appelle run, qui va appeler la fonction qui l'a initialement appelée.
            # -----------------------------------------------------------------------------
            # Call run, which will call the function that originally invoked it.
            self.window.after(0, self.run)
        else:
            # On incrémente current_container_action_index car on va exécuter l'action suivante.
            # ---------------------------------------------------------------------------
            # Increment current_container_action_index because the next action is going to be executed.
            self.current_container_action_index += 1

            # On regarde si l'action que l'on doit exécuter n'a pas été désactivée
            # par l'utilisateur dans l'arborescence.
            # ------------------------------------------------------------------------
            # Check whether the action that is about to be executed has been disabled
            # by the user in the tree structure.
            if list(self.container_action_checkbuttons[self.current_container_action_index-1])[0].get() == 0:
                # On passe à l'action suivante car celle-ci a été désactivée.
                # ------------------------------------------------------------
                ...
                if isinstance(self.container_actions_dict[self.current_container_action_index-1], Loop) or isinstance(self.container_actions_dict[self.current_container_action_index-1], ParallelActions):
                    ...
                    # rappeler la fonction run_action une fois quelle a fini d'executer les actions 
                    # qu'elle contient et on donne aussi le temps de pause entre chaque action.
                    # --------------------------------------------------------------------------
                    # Pass the run function to run_action so that it can
                    # call run_action again once it has finished executing the actions
                    # it contains, and also pass the pause time between each action.
                    ...
                    # On récupère le temps d'attente avant d'appeler l'action suivante,
                    # qu'elle retourne si elle en a un.
                    # ------------------------------------------------------------------
                    # If the action is not a container, execute the action
                    # Retrieve the waiting time before calling the next action,
                    # if it returns one.
                    time = self.container_actions_dict[self.current_container_action_index-1].run()
                    ...
                    # appeler call_release_keys après le temps d'attente choisi par l'utilisateur.
                    # -----------------------------------------------------------------------------
                    # Check whether the action that has just been executed was KeyPress
                    # in order to call call_release_keys after the waiting time chosen by the user.
                    if isinstance(self.container_actions_dict[self.current_container_action_index-1], KeyPress):
                        self.scheduled_task_id = self.window.after(time, self.call_release_keys)

                    # On regarde si l'action que l'on vient d'exécuter était Wait pour pouvoir 
                    # rappeler run_action après le temps d'attente choisi par l'utilisateur.
                    # -------------------------------------------------------------------------
                    # Check whether the action that has just been executed was Wait
                    # in order to call run_action again after the waiting time chosen by the user.
                    elif isinstance(self.container_actions_dict[self.current_container_action_index-1], Wait):
                        self.scheduled_task_id = self.window.after(time+self.pause_between_actions, self.run_action)

                    else:
                        # L'action que nous venons d'appeler ne nécessite pas de temps d'attente, alors 
                        # nous appelons run_action pour exécuter l'action suivante.
                        # ------------------------------------------------------------------------------
                        # The action that has just been called does not require any waiting time,
                        # so call run_action to execute the next action.
                        self.window.after(self.pause_between_actions, self.run_action)
    ...
    def call_release_keys(self) -> None:
        """
        Cette fonction appelle release_keys pour KeyPress afin de relâcher les touches précédemment 
        pressées, puis on appelle run_action pour exécuter l'action d'après.
        ----------------------------------------------------------------------------------------------
        This function calls release_keys for KeyPress in order to release the keys that were previously
        pressed, then calls run_action to execute the next action.
        """
        self.container_actions_dict[self.current_container_action_index-1].release_keys()
        self.window.after(self.pause_between_actions, self.run_action)

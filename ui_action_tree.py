...
from enums import LEFT_CONTAINER
...
class TreeUI:
    ...
    """
    def __init__(self, window: tk.Tk, menu_btn_frame: tk.Frame, switch_frame: Callable[[tk.Frame], None], w_var: WindowVariable, reset_action_sequence: Callable[[None], None]):
        """
        Initialisation de tous les widgets nécessaires à ce menu.

        :param window: Fenêtre principale dans laquelle se trouve ce menu
        ...
        :param window: Main window in which this menu is located
        ...
        self.wait_time_config_frame = tk.Frame(self.tree_frame, bg=w_var.color_1)
        ...
        self.wait_duration_entry.grid(row=1, column=0)
        ...
        self.validate_wait_button.grid(row=1, column=2, padx=10)
        ...
        self.control_buttons_frame = tk.Frame(self.tree_frame, bg=w_var.color_1)
        ...
        self.save_frame = tk.Frame(self.tree_frame, bg=w_var.color_1)
        ...
        self.save_validate_button = tk.Button(
            self.save_frame, 
            text="validate", 
            bg=w_var.color_2, 
            fg=w_var.color_3, 
        ...
        self.save_validate_button.grid(row=3, column=0)
        ...
        # Frame pour organiser les boutons de suppression, modification et sortie de l'édition.
        # --------------------------------------------------------------------------------------
        # Frame used to organize the delete, edit, and exit editing buttons.
        self.edit_control_buttons_frame = tk.Frame(self.edit_tree_frame, bg=w_var.color_1)
        ...
        self.edit_action_button = tk.Button(
            self.edit_control_buttons_frame, 
            text="edit", 
            bg=w_var.color_2, 
            fg=w_var.color_3, 
        ...
        self.leave_edit_button = tk.Button(
            self.edit_control_buttons_frame, 
            text="left", 
            bg=w_var.color_2, 
            fg=w_var.color_3, 
        ...
        self.leave_edit_button.grid(row=0, column=2)
        ...
        self.click_frame = tk.Frame(self.edit_act_frame, bg=self.w_var.color_1)
        self.move_frame = tk.Frame(self.edit_act_frame, bg=self.w_var.color_1)
        self.write_frame = tk.Frame(self.edit_act_frame, bg=self.w_var.color_1)
        self.scroll_frame = tk.Frame(self.edit_act_frame, bg=self.w_var.color_1)
        self.wait_frame = tk.Frame(self.edit_act_frame, bg=self.w_var.color_1)
        self.pressed_frame = tk.Frame(self.edit_act_frame, bg=self.w_var.color_1)
        self.parallel_actions_frame = tk.Frame(self.edit_act_frame, bg=self.w_var.color_1)
        self.loop_frame = tk.Frame(self.edit_act_frame, bg=self.w_var.color_1)

        # Bouton de validation de l'édition de chaque action.
        # ----------------------------------------------------
        # Button used to validate the editing of each action.
        self.validate_button_edit = tk.Button(
            self.edit_act_frame, 
            text="validate", 
            bg=w_var.color_2, 
            fg=w_var.color_3, 
    ...
    def frame_act(self) -> None:
        ...
        self.click_ui = ClickFrame(self.click_frame, self.w_var)
        self.move_ui = MouseMoveFrame(self.move_frame, self.w_var)
        self.write_ui = WriteFrame(self.write_frame, self.w_var)
        self.scroll_ui = ScrollFrame(self.scroll_frame, self.w_var)
        self.wait_ui = WaitFrame(self.wait_frame, self.w_var)
        self.pressed_ui = KeyHoldFrame(self.pressed_frame, self.w_var)
        self.parallel_actions_ui = ParallelActionsFrame(self.parallel_actions_frame, self.w_var, self.container_manager)
        self.loop_ui = LoopFrame(self.loop_frame, self.w_var, self.container_manager)
    ...
    def index_save(self, dict_edit_index: int) -> int:
        ...
        while element_idx <= real_index_save:
            if self.ui_action_manager.action_manager.save_list[element_idx][0] == LEFT_CONTAINER:
                real_index_save+=1
            element_idx += 1
    ...
    def call_save(self) -> None:
        ...
        if not self.ui_action_manager.action_manager.is_running:
            ...
            if msg_error != True:
                ...
                self.save_error_label.grid(row=2, column=0)
            else:
    ...
    def add_tree(self, text: str) -> None:
        ...
        action_checkbutton = tk.Checkbutton(self.all_actions.scrollable_frame, text=text, variable=check_var, bg=self.w_var.color_1)
